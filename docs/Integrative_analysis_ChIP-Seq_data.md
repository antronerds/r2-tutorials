...
![](_static/images/IntgrAnalysis_ChipSeq/IntAnalysis_ChIPSeq_Transcription.png)

[**Figure 1: Transcription; taken from Nature Reviews Genetics 12, 283-293 (April 2011)**](_static/images/IntgrAnalysis_ChipSeq/IntAnalysis_ChIPSeq_Transcription.png)
...
![](_static/images/IntgrAnalysis_ChipSeq/IntAnalysis_ChIPSeq_ModificationTypes.png)

[**Figure 2: Specific chemical modifications mark specific states of cis-regulatory elements; taken from doi:10.1016/j.molcel.2013.01.038**](_static/images/IntgrAnalysis_ChipSeq/IntAnalysis_ChIPSeq_ModificationTypes.png)
...
![](_static/images/IntgrAnalysis_ChipSeq/IntAnalysis_ChIPSeq_InflectionPoint.png)

[**Figure 3: Result of a typical ROSE analysis. Above the inflection point, marked in red, are super-enhancer regions**](_static/images/IntgrAnalysis_ChipSeq/IntAnalysis_ChIPSeq_InflectionPoint.png)
...
1. For this example check if the correct dataset has been selected in this case "Tumor Neuroblastoma public- Versteeg -88".  Note that you have to select the correct dataset set before starting with the Chip-seq analyses. To enter the ChIP-seq analysis module in R2 select *ChIP data* in the left side menu.
	
	![](_static/images/IntgrAnalysis_ChipSeq/IntAnalysis_ChIPSeq_chipanalyses_leftmenu.png)
	
	[**Figure 4: Choose the ChIP-seq module**](_static/images/IntgrAnalysis_ChipSeq/IntAnalysis_ChIPSeq_chipanalyses_leftmenu.png)
...
2. See figure 5. Several analysis paths start from here. First we're going to explore the genomic environment of some genes in context of ChIP-seq data. In the ChIP-seq menu choose the *ChIP-chip Genome Browser* **(+GEX)**
	...
	![](_static/images/IntgrAnalysis_ChipSeq/IntAnalysis_ChIPSeq_ChIPSeqMenu_b.png)
	
	[**Figure 5: ChIP-seq Menu in R2**](_static/images/IntgrAnalysis_ChipSeq/IntAnalysis_ChIPSeq_ChIPSeqMenu_a.png)
...
2. To select the proper transcript in the next screen, click the "View" button. 
	...
	![](_static/images/IntgrAnalysis_ChipSeq/IntAnalysis_ChIPSeq_GATA3_select.png)
	
	[**Figure 6: Looking up a single gene in the R2 Genome Browser in ChIP-seq context**](_static/images/IntgrAnalysis_ChipSeq/IntAnalysis_ChIPSeq_GATA3_select.png).
...
3. To select one or more ChIP-seq datasets, click "Select/Adapt ChIP-Experiments" in the *Properties* panel on the left. As an example we write "lan1" in the text field of the *chip_celline* column. Check the box in front of the preferred experiment(s), optionally change the display colors using the "c.c." buttons on the right and click "Update" at the bottom. Before we redraw the display, we adjust some additional settings in step 4.  
	...
	![](_static/images/IntgrAnalysis_ChipSeq/IntAnalysis_ChIPSeq_SelectLan1_profiles.png)
	
	[**Figure 7: Selecting experiments by using grid filtering**](_static/images/IntgrAnalysis_ChipSeq/IntAnalysis_ChIPSeq_SelectLan1_profiles.png)
...
4. In the *Tracks* panel on the right different annotation settings can be chosen. In Fig 8 we first adjusted the TranscriptView Annotation settings:  The *NIH Epigenome Roadmap* to 'all' and the *SuperEnhancers NB (George)* to 'on'. Next to the dropdown menus a toolset icon gives access to alternative displays of the information (e.g. a more detailed display per cell line can be chosen for the NIH Epigenome Roadmap information in stead of the cell line aggregated information obtained with 'all').  
	...
	![](_static/images/IntgrAnalysis_ChipSeq/IntAnalysis_ChIPSeq_GATA3_b.png)
	
	[**Figure 8: Histone modification profiles around GATA3 location**](_static/images/IntgrAnalysis_ChipSeq/IntAnalysis_ChIPSeq_GATA3_b.png)
...
7. Set perspective to peaks if it was not done so already. Copy paste the genes obtained in step 5 or type genes of your interest into the *Enter genesymbols / genome positions* textbox.  In the *Gene Order* selection box select 'by_row_signal' and click "Next". The Gata binding sites around the genes in the list are shown (Figure 9). 
	...
	![](_static/images/IntgrAnalysis_ChipSeq/IntAnalysis_ChIPSeq_ExpSelect_a.png)
	
	[**Figure 9: Gata binding site around genes**](_static/images/IntgrAnalysis_ChipSeq/IntAnalysis_ChIPSeq_ExpSelect_a.png)
...
8. Since the ordering of the ChIPSeq Peak Plotter lists the genes with the highest signals on top  (due to *Gene order* set to 'by_row_signal'), we'll select one of the first listed genes; click on ALK, in a new tab the GATA3 binding signal at the gene location is plotted in the R2 Genome Browser (Figure 10). 
	...
	![](_static/images/IntgrAnalysis_ChipSeq/IntAnalysis_ChIPSeq_GATA3_ALKprofile.png)
	
	[**Figure 10: ALK profile within GATA3 ChIP-seq experiment**](_static/images/IntgrAnalysis_ChipSeq/IntAnalysis_ChIPSeq_GATA3_ALKprofile.png)
	...
	![](_static/images/IntgrAnalysis_ChipSeq/IntAnalysis_ChIPSeq_Alk_GATA3_oldridge.png)
	
	[**Figure 11: GATA3 binding data around the ALK gene in multiple datasets**](_static/images/IntgrAnalysis_ChipSeq/IntAnalysis_ChIPSeq_Alk_GATA3_oldridge.png)
...
3. R2 now shows for all provided genes a 100 Kb region up and downstream of the TSS. Note that the genebody of ALK and BRD4 are colored green and red respectively. Projected on the stretch are the bins that the Rseg-ROSE algorithm considers super-enhancers (Figure 12). Each stretch is clickable and will open a new tab. Click the topmost gene.
	...
	![](_static/images/IntgrAnalysis_ChipSeq/IntAnalysis_ChIPSeq_list_RSEG.png)
	
	[**Figure 12: Histone acetylation around the TSS of a set of genes**](_static/images/IntgrAnalysis_ChipSeq/IntAnalysis_ChIPSeq_list_RSEG.png)
...
4. For the topmost gene the acetylation data is shown on the chosen stretch. To further analyze what's going on we'll add GATA3 binding data and methylation data for the same cell line by checking the appropriate boxes. Click "redraw". Note especially the region to the right where a super-enhancer is located, methylation signal is lower and there is not much GATA binding (Figure 13).
	...
	![](_static/images/IntgrAnalysis_ChipSeq/IntAnalysis_ChIPSeq_HistoneAcetylation_for_topgene_a.png)
	
	[**Figure 13: ChIP-seq signals around the TSS of a single gene**](_static/images/IntgrAnalysis_ChipSeq/IntAnalysis_ChIPSeq_HistoneAcetylation_for_topgene_a.png)
...
3. Again select the SY5Y dataset from Oldridge in the next panel
	...
	![](_static/images/IntgrAnalysis_ChipSeq/IntAnalysis_ChIPSeq_Rose_select.png)
	
	[**Figure 14: Selecting super-enhancers from an interactive ROSE plot**](_static/images/IntgrAnalysis_ChipSeq/IntAnalysis_ChIPSeq_Rose_select.png)
...
4. R2 shows an interactive ROSE plot (Figure 15); dots in red are clickable and represent areas on the genome that ROSE has assigned as super-enhancer. Click one of them. In this example the 5th ranked enhancer was chosen.
	...
	![](_static/images/IntgrAnalysis_ChipSeq/IntAnalysis_ChIPSeq_Rose_Genomebr.png)
	
	[**Figure 15: Selecting super-enhancers from an interactive ROSE plot**](_static/images/IntgrAnalysis_ChipSeq/IntAnalysis_ChIPSeq_Rose_Genomebr.png)
...
5. R2 opens a new panel showing the location on the genome of the super-enhancer. To further explore which genes might be influenced, rescale the signal to a value of 150 (*Range*), put *Slider* to 'median' with value 5 and zoom out. The resulting picture shows that there are several genes in the proximity (Figure 16). Also present are other super-enhancers nearby. Feel free to toy around with the settings, and corroborate your findings by showing additional datasets in the same region.
	...
	![](_static/images/IntgrAnalysis_ChipSeq/IntAnalysis_ChIPSeq_Rose_Genomebradapt.png)
	
	[**Figure 16: The genomic context of a top-ranking super-enhancer**](_static/images/IntgrAnalysis_ChipSeq/IntAnalysis_ChIPSeq_Rose_Genomebradapt.png)
